...
def jugadaComputadora(tablero,numero_jugada):
    ...
    for e in posibles:
        ...
        resultados.append(mejorJugada(copTabl,e))   #Calculamos las valoraciones de las jugadas posibles

    pos = resultados.index(max(resultados))         #Elegimos la que tiene el mejor resultado para la computadora
    end= time.time()
    print('Tiempo de calculo de jugada: ',end-start)
    return posibles[pos]

def mejorJugada(tablero,jugada):
    ...
    for e in posibles:
        copTabl = deepcopy(tablero)
        valor = mejorJugadaPersona(copTabl,e)
        if valor == -1:
            return -1

        resultados.append(valor)    #Calculo las jugadas de la persona

    return min(resultados)          #Supongo que elige la mejor para el

def mejorJugadaPersona(tablero,jugada):    
    ...
    for e in posibles:
        copTabl = deepcopy(tablero)
        valor = mejorJugada(copTabl,e)
        if valor == 1:
            return 1
        resultados.append(valor)    #Calculo las jugadas de la computadora
    
    return max(resultados)          #Supongo que elige la mejor para ella
